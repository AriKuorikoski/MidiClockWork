...
class ValetonTempoHandler(TempoHandler):
    ...
    def handle(self, bpm, writer):
        result = self._bpm_to_cc(bpm)
        if result is None:
            return
        msb, lsb = result
        writer.process(MidiMessage(CC, self.channel, _CC_TEMPO_MSB, msb))
        writer.process(MidiMessage(CC, self.channel, _CC_TEMPO_LSB, lsb))
